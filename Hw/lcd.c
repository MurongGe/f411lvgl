...
void lcd_showchar(uint16_t x, uint16_t y, uint8_t size, uint8_t mode, unsigned char ch)    //���ٻ���ʵ��
{
...
void lcd_scanchar(uint16_t x, uint16_t y, uint8_t size, unsigned char ch)
{
  ...
  /* start send pox */
  LCD_WR_REG(0x2C);
  LCD_WR_DATAS(size * size / 2, lcdbuffer);
}
...
void lcd_fill_color(uint16_t sx, uint16_t ex, uint16_t sy, uint16_t ey, uint8_t *color)
{
  ...
  LCD_WR_REG(0x2C);   //memory write
  for (i = 0; i < times; i++) {
    LCD_WR_DATAS(DMATRANS_MAX_NUM, color + DMATRANS_MAX_NUM * i);
  }
  LCD_WR_DATAS(left, color + DMATRANS_MAX_NUM * i);
}